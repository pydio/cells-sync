...
func parseUrl(urlArg string, otherArgs []string) (ep model.Endpoint, e error) {
	...
	} else if u.Scheme == "router" {
		return endpoint.NewRouterEndpoint(strings.TrimLeft(u.Path, "/")), nil
...
func parseArgs(args []string) (model.PathSyncSource, model.PathSyncTarget) {
	...
	return interface{}(source).(model.PathSyncSource), target.(model.PathSyncTarget)
}
		...
		for {
			...
			case processorEvent := <-processorEvents:
				if processorEvent.Type == "merger:end" {
					...
					diff, err := merger.ComputeDiff(ctx, source, target.(model.PathSyncSource), nil)
					if err != nil {
